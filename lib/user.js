...
User.prototype.is_qualified = function () {
  return this._is_qualified(this.traits().customer_fit_segment)
}
...
User.prototype.qualify = function (fn) {
  ...
  this.persons((err, res) => {
    ...
    res = res || {}
    var customer_fit_segment = res.properties && res.properties.customer_fit && res.properties.customer_fit.segment ? res.properties.customer_fit.segment : 'not enough information'
    var traits = {
      domain: res.properties && res.properties.domain ? res.properties.domain : null,
      customer_fit_segment: customer_fit_segment,
      qualified: this._is_qualified(customer_fit_segment),
      company: res.company || {},
      person: (function () {
        delete res.company
        return res
