...
MadKudu.prototype.normalize = function (msg) {
  ...
  msg.messageId = 'mjs-' + md5(window.JSON.stringify(msg) + uuid())
...
MadKudu.prototype.send = function (route, data, fn = noop) {
  const url = this.api_host + route
  ...
  if (this.options.beacon && navigator.sendBeacon) {
    ...
    if (navigator.sendBeacon(url, window.JSON.stringify(msg))) {
...
MadKudu.prototype.request = function (route, data, fn) {
  ...
  this._send_get(url, data, headers, (err, res) => {
    if (err) { return fn(err) }
    try {
      var response = window.JSON.parse(res.response)
      fn(null, response)
    } catch (e) {
      return fn(e)
    }
